# 🛠️ Desarrollo — Índice

> Índice de la carpeta 02 - Desarrollo.

## Notas principales

- [Backend](Backend.md) — Lógica del lado servidor (Python, Flask/FastAPI).
- [Frontend](Frontend.md) — Interfaz web responsive.
- [API](API.md) — Comunicación REST.
- [Base de Datos](Base%20de%20Datos.md) — SQLite → PostgreSQL.
- [Configuración](Configuracion.md) — Variables de entorno y puesta en marcha.
- [Dashboard](Dashboard.md) — Centro de control.

## Gestión de bots

- [Bots](Bots.md) — Vista general.
- [Configuracion de Bots](Configuracion%20de%20Bots.md) — Cómo se define cada bot.
- [Acciones de Bots](Acciones%20de%20Bots.md) — Operaciones posibles.
- [Auto Restart](Auto%20Restart.md) — Reinicio automático.

## Gestión de webs

- [Webs](Webs.md) — Vista general.
- [Gestion Avanzada de Webs](Gestion%20Avanzada%20de%20Webs.md) — Despliegue futuro.

## Servidor y sistema

- [Servidor](Servidor.md) — Laptop con Debian 13.
- [Control Remoto](Control%20Remoto.md) — Ejecución de acciones.

## Conceptos

- [Sistema Adaptable](Sistema%20Adaptable.md) — Detección automática de configuración.
- [Notificaciones](Notificaciones.md) — Alertas futuras.
- [Git y GitHub](Git%20y%20GitHub.md) — Control de versiones.
- [Documentacion](Documentacion.md) — Gestión de la documentación.

## Despliegue

- [Despliegue/Índice](Despliegue/%C3%8Dndice.md) — Guías de puesta en producción.
  - [Docker](Despliegue/Docker.md) — Imagen slim y Alpine, panel y agente multi-init.
